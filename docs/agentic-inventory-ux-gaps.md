...
- [x] **[AC#121](https://github.com/konveyor/agentic-controller/issues/121) CLOSED 2026-08-18 — resolved application-only for the preview, no archetype label on runs.** The Option A proposal's unrecoverability argument only holds *if* archetype-context launches exist; with the association cut (next item) and phase 1 = single-app, the archetype is a selection filter and every run carries the application label, so there is nothing to stamp and nothing lost (membership stays derivable from the app id). djzager's nudge ("a thing the Hub does?") is right — a future stamp + `?archetype=` selector is Hub create-path work, so the reopen target is a Hub issue, not AC. Runs chips and the hub filter surface are application-only; [hub#1112](https://github.com/konveyor/tackle2-hub/issues/1112) acceptance item 1 amended the same day. Ramon confirmation never landed explicitly — covered by the 08-11 phase-1 agreement.
...
- [x] **Land parent-label inheritance for stage runs** — [PR#113](https://github.com/konveyor/agentic-controller/pull/113) MERGED 2026-08-13; stage runs now inherit the parent's labels, so application-label filtering covers workflow stages.
...
- [x] **Wire the real preflight checks** — DONE 2026-08-13, browser-verified on the mock rig. The bulk modal now blocks on required-defaultless params across the workflow's stage agents ("Stage assess (migration-analyzer) requires parameters this launch cannot supply: repository") and on dangling stage agentRefs, and warns per-app when no source-role credential is assigned ("1 of 2 eligible applications have no source credential"). Block-vs-warn hardening stays with the UX preflight design above; the credential check reads the app's identities roles, so a global default identity shows as a (conservative) warning.
- [ ] **Plumb failure reasons** from the stage pod into run status so the UI has something to render — issue text DRAFTED 2026-08-13 (termination-message design: `FallbackToLogsOnError` → AgentRun Failed-condition message → workflow propagation), posting to agentic-controller owed.
- [x] **App detail: button to the pre-filtered runs page** — DONE 2026-08-13 ([ui#3521](https://github.com/konveyor/tackle2-ui/issues/3521)): "Workflow runs" joins Issues/Insights/Dependencies in the drawer's Details cross-link list, deep-linking to `/workflow-runs?awr:filters={"application":[name]}`; the fetch-everything drawer tab is deleted.
- [x] **Bulk landing pre-filtered** — DONE 2026-08-13 ([ui#3520](https://github.com/konveyor/tackle2-ui/issues/3520)): submit navigates straight to the runs page with Application chips for the launched apps plus the new Workflow chip (category added to the runs page), and the toast's "View runs" repeats the same URL.
- [x] **Steer behind a default-off flag** — DONE 2026-08-13 ([ui#3527](https://github.com/konveyor/tackle2-ui/issues/3527)): `AGENTIC_STEER_ENABLED` (default false in serverConfig, on in local dev) gates the chat MessageBar; verified both positions on the prod-mode rig. Permission prompts stay answerable — solicited asks hang the turn otherwise. NOTE: the ROKS build stays steer-exposed until the image is bumped.
- [ ] **Finish the review pass owed on [hub#1112](https://github.com/konveyor/tackle2-hub/issues/1112)** so a server-side `?application=` filter can land under the URLs the UI already emits (`?archetype=` dropped 2026-08-18 with #121's closure; client-side first per the 08-10 call, so not preview-blocking).
...
## 3. AC#121 decision record (posted 2026-08-10 — SUPERSEDED 2026-08-18: issue closed application-only, no stamp; the analysis below stays valid for *if* an archetype-subject launch path ever appears, and then belongs in a Hub issue)
...
Add to `workflow-runs-page.tsx`: an Application column (resolve via run labels once stamping lands, APP_ID-env fallback meanwhile) and a `FilterToolbar` with chips for application, workflow, phase, ~~and archetype (archetype chip gated on #121 stamping)~~ (#121 closed 2026-08-18: no archetype chip). Every launch path (2A) and the app-detail affordance (B6: a button to this page with the application chip pre-applied, showing **workflow** runs, not stage runs) routes here via URL filter serialization. Client-side filtering at preview scale; upgrade to hub#1112 server filters without UX change. Effort M. This page is what ui#3520/#3521/#3523 all point at.
...
1. **AC#121 decision — now, before any preview code.** Zero implementation cost to decide; Option A's provenance data cannot be backfilled. Every run created before stamping lands is permanently invisible to the archetype filter (the shim already documents the same caveat for the application label). *(Resolved 2026-08-18: closed application-only — no stamping; nothing to backfill because no archetype-subject launch exists in the preview.)*
...
4. **hub#1112 run routes + filters (B2).** `?application=` per the shim reference semantics (unsupported filter ⇒ 400), ~~`?archetype=` as the same label-selector path once #121/A lands~~ (dropped 2026-08-18 with #121's closure). Create-path stamping of ~~both labels~~ the application label lives here too.
